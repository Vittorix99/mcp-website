...
def admin_mailerlite_automations(req):
    if req.method == "GET":
        ...
        if automation_id:
            try:
                return automations_client.get(automation_id), 200
            except MailerLiteError as e:
                return handle_mailerlite_error(e)
        try:
            return automations_client.list(params=params), 200
        except MailerLiteError as e:
            return handle_mailerlite_error(e)
...
def admin_mailerlite_automation_activity(req):
    ...
    params.pop("id", None)
    try:
        return automations_client.activity(automation_id, params=params), 200
    except MailerLiteError as e:
        return handle_mailerlite_error(e)
