...
sequenceDiagram
    ...
    S->>R: stream_models()
    R->>FS: events.where(status=active).stream()
    FS-->>R: documenti
    R-->>S: Event[]
...
sequenceDiagram
    ...
    EP->>FS: memberships.where(email in emails)
...
sequenceDiagram
    ...
    API->>NS: signup(data)
    NS->>FS: newsletter_signups.where(email==) 
    alt Già esistente
        NS-->>API: ok (noop)
    else Nuovo
...
sequenceDiagram
    ...
    PR->>FS: collectionGroup(participants_event)<br/>where(membershipId==source)<br/>update membershipId=target
...
sequenceDiagram
    ...
    TR->>FS: memberships.where(subscription_valid=true)
    loop batch di 500
...
sequenceDiagram
    ...
    alt event ∈ {unsubscribed, bounced, spam}
        ...
        NR->>FS: newsletter_signups<br/>where(email==).update(active=false)
        NR->>FS: newsletter_consents<br/>where(email==).update(status=...)
    end
