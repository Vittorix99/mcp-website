...
function CreateFieldModal({ onClose, onCreated }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setError("Errore durante la creazione del campo.")
    } finally {
      setLoading(false)
    }
  }
...
export default function SenderFieldsPage() {
  ...
  async function handleDelete(f) {
    if (!confirm(`Eliminare il campo "${f.title}"?\n\nAttenzione: questa azione rimuove il campo da tutti i subscriber.`)) return
                ...
                {loading ? (
                  ...
                  <TableRow className="border-neutral-800">
                    <TableCell colSpan={3} className="text-center text-neutral-500 py-12">
                      Nessun campo personalizzato. Creane uno per arricchire i profili subscriber.
                    </TableCell>
                  </TableRow>
                ) : (
                  fields.map((f) => (
                    <TableRow key={f.id} className="border-neutral-800 hover:bg-white/5">
